...
| `AokiPrimeTheorem` | EXPLICIT INPUT TYPE | Aoki's converse is supplied as theorem data with its prime, nonzero-entry, even-length, and all-row balance premises exposed; it is not asserted globally. |
...
| P2.12 Prime relations are opposite-paired | PARTIAL FORMALIZED + TYPED SOURCE INTERFACE | Relation-to-balance and opposite-paired-to-balance are formalized arithmetically. Balanced-to-opposite-paired is exposed only through `AokiPrimeTheorem`, including Aoki's even-length premise. The 1984 erratum was checked and changes only Theorem B, not the imported Theorem A. |
