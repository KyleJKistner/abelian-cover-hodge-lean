...
The eight source files listed below are unmodified copies of the recovered
package. The new `manuscripts/phase_I_revised.tex` and
`manuscripts/fusion_revised.tex`, their review PDFs, the repair documentation
and new certificate scripts are September 2026 revisions. The `complete`
filenames and frozen ledgers retain their historical wording; they do not
represent the current claim boundary. The Lean layer proves a separately
specified arithmetic and combinatorial subset.
...
- **Original Phase II Python layer:** regenerated byte for byte on Python
  3.14.6, SymPy 1.14.0 and mpmath 1.3.0. `requirements-audit.txt` pins both
  Python packages. The runner stages the unchanged script and source files in
  a temporary directory and requires the reproduced report's SHA-256 to match
  the frozen value in the table. The frozen report is never overwritten.
...
- **Combined replay:** `scripts/replay_certificates.py` checks all eight frozen
  ...
  same entry point. Runtime-specific reports are regenerated, not committed.
...
The revised fusion manuscript additionally pins ACV balanced smoothing,
compact-type Picard, Schoen's original result and 1998 addendum, and cycle-class
specialization. Its exact source checks and qualifications are recorded in
`FUSION_REPAIR.md`; they are written mathematical source matches, not completed
Lean interfaces. The particular prospective Lean contexts for these results,
